...
abstract contract BumpingFee is BaseHook {
    ...
    function beforeSwap(
        ...
        bytes calldata
    ) public virtual view override returns (bytes4, BeforeSwapDelta, uint24) {
    ...
    function afterSwap(
        ...
        BalanceDelta,
        bytes calldata
    ) public virtual override returns (bytes4, int128) {
        manager.updateDynamicLPFee(key, uint24(START_FEE));
        startTimestamp = block.timestamp;
